...
import {
  ...
  BookOpen, Heart, BookMarked, Sparkles, UserRoundSearch,
} from "lucide-react";
...
import {
  ...
  blockUser, unblockUser, removeFriend, getActivityFeed,
...
function FriendAvatar({ name, avatarUrl, className }: { name: string; avatarUrl?: string | null; className?: string }) {
  return (
    <Avatar className={cn("size-10 rounded-xl ring-2 ring-border shrink-0", className)}>
      {avatarUrl && <AvatarImage src={`${AVATAR_BASE}/uploads/avatars/${avatarUrl}`} alt={name} className="rounded-xl object-cover" />}
...
function UserInfo({ name, lastname, alias }: { name: string; lastname: string; alias?: string | null }) {
  return (
    <div className="flex flex-col min-w-0 leading-tight">
      <span className="text-sm font-medium truncate text-foreground">{name} {lastname}</span>
      {alias && <span className="text-xs text-muted-foreground/70 truncate">@{alias}</span>}
    </div>
  );
}

function FriendCardWrapper({ children, className, style }: { children: React.ReactNode; className?: string; style?: React.CSSProperties }) {
  return (
    <div
      className={cn(
...
function ActivityItem({ activity, index }: { activity: FriendActivity; index: number }) {
  const { user, event, metadata, createdAt } = activity;
  const fullName = `${user.name} ${user.lastname}`;
  const icon = event === "MARK_READ"
    ? <BookOpen className="size-3.5" />
    : event === "ADD_FAVORITE"
      ? <Heart className="size-3.5" />
      : <BookMarked className="size-3.5" />;

  const accentBorder = event === "MARK_READ"
    ...
    >
      ...
      <div className="flex flex-col min-w-0 gap-0.5 flex-1">
        ...
        <p className="text-xs text-muted-foreground leading-snug">
          {event === "MARK_READ"
            ? `Leyó ${metadata.chapterName ?? "un capítulo"} de ${metadata.seriesName ?? "una serie"}`
            : event === "ADD_FAVORITE"
          ...
          {results.map((user) => (
            <FriendCardWrapper key={user.id}>
              ...
              {user._friendStatus === "ACCEPTED" ? (
                ...
                <Button size="sm" variant="outline" className="shrink-0 gap-1.5 transition-all duration-200 hover:border-brand/40 hover:text-brand hover:bg-brand/5" onClick={() => handleSendRequest(user.id)}>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
      {friends.map((friend, i) => (
        <FriendCardWrapper key={friend.id} style={{ animationDelay: `${i * 30}ms` }}>
          <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex gap-1 shrink-0">
            <Button size="icon" variant="ghost" className="size-8 text-muted-foreground/60 hover:text-destructive hover:bg-destructive/10 transition-all" onClick={() => handleRemove(friend.id)} title="Eliminar amigo">
              <UserX className="size-4" />
            </Button>
            <Button size="icon" variant="ghost" className="size-8 text-muted-foreground/60 hover:text-destructive hover:bg-destructive/10 transition-all" onClick={() => handleBlock(friend.id)} title="Bloquear usuario">
...
function ReceivedRequests() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
      {requests.map((req, i) => (
        <FriendCardWrapper key={req.id} style={{ animationDelay: `${i * 30}ms` }}>
          <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex gap-1.5 shrink-0">
            <Button size="sm" variant="default" className="gap-1 h-8 transition-all duration-200" onClick={() => handleAccept(req.id)}>
              <UserCheck className="size-3.5" /> Aceptar
            </Button>
            <Button size="sm" variant="ghost" className="gap-1 h-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all" onClick={() => handleReject(req.id)}>
...
function BlockedUsersList() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
      {blocked.map((b, i) => (
        <FriendCardWrapper key={b.id} style={{ animationDelay: `${i * 30}ms` }}>
          <div className="flex items-center gap-3 min-w-0">
          ...
          <Button size="sm" variant="outline" className="shrink-0 h-8 transition-all duration-200 hover:border-brand/40 hover:text-brand" onClick={() => handleUnblock(b.user.id)}>
            Desbloquear
          </Button>
        </FriendCardWrapper>
      ))}
            ...
            <div className="flex justify-center min-w-0">
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col leading-tight">
                  ...
                  <span className="text-[10px] text-muted-foreground/60 hidden sm:block">Conecta con otros lectores</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <main className="container mx-auto px-4 py-6">
          <Tabs defaultValue="activity" className="w-full">
            <TabsList className="w-full h-auto p-1 bg-muted/50 rounded-2xl flex-wrap mb-6 gap-0.5">
              <TabsTrigger value="activity" className="flex-1 gap-1.5 rounded-xl text-xs data-[state=active]:bg-background data-[state=active]:shadow-sm py-2 min-w-0">
                <Sparkles className="size-3.5 shrink-0" /> <span className="truncate">Actividad</span>
              </TabsTrigger>
              <TabsTrigger value="friends" className="flex-1 gap-1.5 rounded-xl text-xs data-[state=active]:bg-background data-[state=active]:shadow-sm py-2 min-w-0">
                <HeartHandshake className="size-3.5 shrink-0" /> <span className="truncate">Amigos</span>
              </TabsTrigger>
              <TabsTrigger value="requests" className="flex-1 gap-1.5 rounded-xl text-xs data-[state=active]:bg-background data-[state=active]:shadow-sm py-2 min-w-0">
                <UserPlus className="size-3.5 shrink-0" /> <span className="truncate">Solicitudes</span>
              </TabsTrigger>
              <TabsTrigger value="sent" className="flex-1 gap-1.5 rounded-xl text-xs data-[state=active]:bg-background data-[state=active]:shadow-sm py-2 min-w-0">
                <Send className="size-3.5 shrink-0" /> <span className="truncate">Enviadas</span>
              </TabsTrigger>
              <TabsTrigger value="search" className="flex-1 gap-1.5 rounded-xl text-xs data-[state=active]:bg-background data-[state=active]:shadow-sm py-2 min-w-0">
                <Search className="size-3.5 shrink-0" /> <span className="truncate">Buscar</span>
              </TabsTrigger>
              <TabsTrigger value="blocked" className="flex-1 gap-1.5 rounded-xl text-xs data-[state=active]:bg-background data-[state=active]:shadow-sm py-2 min-w-0">
                <Ban className="size-3.5 shrink-0" /> <span className="truncate">Bloqueados</span>
              </TabsTrigger>
            </TabsList>
