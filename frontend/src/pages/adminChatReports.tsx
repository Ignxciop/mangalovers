...
import type { ChatReport, ReportStatus } from "@/types/admin";
...
export default function AdminChatReports() {
    ...
    async function handleResolve(reportId: number, status: ReportStatus) {
        try {
            await resolveChatReport(reportId, status, adminNote || undefined);
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-24 gap-4 text-center flex-1">
                        <div className="size-14 rounded-full bg-muted/30 flex items-center justify-center">
                            <MessagesSquare className="size-7 text-muted-foreground/30" />
                        </div>
                        <div className="space-y-1.5">
                            <p className="text-base font-medium text-muted-foreground/70">
                                {statusFilter || reasonFilter ? "Sin resultados" : "Sin reportes de chat"}
                            </p>
                            <p className="text-sm text-muted-foreground/50">
                                {statusFilter || reasonFilter ? "Prueba con otros filtros" : "Los reportes de mensajes del chat aparecerán aquí"}
                            </p>
                        </div>
                    </div>
                ) : (
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-border bg-muted/20">
                                            ...
                                            <th className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase tracking-wider">Mensaje</th>
                                        ...
                                        {reports.map((report) => (
                                            <tr key={report.id} className="hover:bg-muted/30 transition-colors">
                                                ...
                                                <td className="px-4 py-3 max-w-[280px]">
                                                    ...
                                                    {report.message && report.message.visible === true && (
                                                ...
                                                <td className="px-4 py-3 text-right">
                                                    {report.status === "PENDING" ? (
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                            onClick={() => { setShowResolveDialog(report.id); setAdminNote(""); setMuteDuration("permanent"); }}
            ...
            {showResolveDialog !== null && createPortal((() => {
                ...
                if (!r) return null;
                const messageVisible = r.message?.visible === true;
                return (
                    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-brightness-[0.3]" onClick={() => setShowResolveDialog(null)}>
                        <div className="bg-background border border-border rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
                            <div className="px-5 py-4 border-b border-border">
                                <h3 className="text-sm font-semibold">Revisar reporte de chat</h3>
                            </div>
                            <div className="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
                                <div className="grid grid-cols-2 gap-3 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium truncate">{r.message?.user?.alias ?? "anónimo"}</p>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="bg-muted/20 rounded-lg p-3 text-sm text-foreground/80 leading-relaxed">
                                        {messageVisible ? r.message?.content : <span className="italic text-muted-foreground/50">Mensaje eliminado</span>}
                                    </div>
                                </div>
                                <div>
