...
function DetailPanel({
    ...
    onStatusChange: (id: number, status: SuggestionStatus, adminResponse?: string) => void;
}) {
    const [responseText, setResponseText] = useState(suggestion.adminResponse ?? "");
        ...
        <div className="space-y-4 text-sm">
            <div className="flex items-start justify-between gap-3">
                ...
                <Select
                    ...
                    onValueChange={(v) => onStatusChange(suggestion.id, v as SuggestionStatus, responseText)}
                >
            ...
            {suggestion.adminResponse && (
                <div className="rounded-lg border border-brand/20 bg-brand/5 p-4 space-y-1.5">
                    ...
                    {suggestion.reviewedBy && (
                        <p className="text-xs text-muted-foreground/70">
                            — {suggestion.reviewedBy.name} {suggestion.reviewedBy.lastname}
                        </p>
                    )}
                </div>
            )}
            ...
            <div className="space-y-2">
                ...
                <textarea
                    ...
                    maxLength={2000}
                    className="flex h-[100px] w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-xs placeholder:text-muted-foreground focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 resize-y overflow-y-auto"
                />
                <p className="text-[11px] text-muted-foreground text-right">{responseText.length}/2000</p>
            </div>
...
export default function AdminSuggestions() {
    ...
    const handleStatusChange = async (id: number, newStatus: SuggestionStatus, adminResponse?: string) => {
        try {
            await updateSuggestionStatus(id, newStatus, adminResponse);
            fetchSuggestions();
